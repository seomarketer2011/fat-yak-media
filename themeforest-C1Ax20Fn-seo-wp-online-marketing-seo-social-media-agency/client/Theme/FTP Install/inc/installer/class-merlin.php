...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$this->slug = apply_filters( 'merlin_slug', $this->slug );
...
function logErrorMessage($message)
{
	...
	public function redirect() {
		...
		do_action( 'merlin_before_redirect' );
	...
	public function admin_page() {
		...
		wp_enqueue_style(
			'merlin',
			trailingslashit( $this->base_url ) . $this->directory . '/assets/css/merlin' . $suffix . '.css',
			array( 'wp-admin' ),
			filemtime( trailingslashit( get_template_directory() ) . $this->directory  . '/assets/css/merlin' . $suffix . '.css' )
		);

		// Enqueue javascript.
		wp_enqueue_script(
			'merlin',
			trailingslashit( $this->base_url ) . $this->directory . '/assets/js/merlin' . $suffix . '.js',
			array( 'jquery-core' ),
			filemtime( trailingslashit( get_template_directory() ) . $this->directory  . '/assets/js/merlin' . $suffix . '.js' ),
			true
		);

		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'seowp' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_attr__( 'Please define default parameters in the form of an array.', 'seowp' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_attr__( 'Please define an SVG icon filename.', 'seowp' );
		}
	...
	public function steps() {
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'seowp' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'seowp' ),
			'view' => array( $this, 'ready' ),
		);

		$this->steps = apply_filters( $this->slug . '_merlin_steps', $this->steps, $this );
	}
	...
	<?php
		$this->logger->debug( esc_attr__( 'The welcome step has been displayed', 'seowp' ) );
		$this->logErrorMessage( '-----------Theme config started----------' );
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( esc_attr__( 'The license activation step has been displayed', 'seowp' ) );
	}
	...
	<?php
		$this->logger->debug( esc_attr__( 'The child theme installation step has been displayed', 'seowp' ) );
	}
	...
	protected function plugins() {
		$this->logErrorMessage('-----------Plugins selected----------');
		...
		$this->logErrorMessage("Plugins List ::".json_encode($plugins));
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'seowp' ); ?>">
										<?php esc_html_e( 'req', 'seowp' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( esc_attr__( 'The plugin installation step has been displayed', 'seowp' ) );
	}
	...
	protected function content() {
		...
		$this->logErrorMessage('-----------Theme content Started---------- ');
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'seowp' ); ?>">
	...
	<?php
		$this->logger->debug( esc_attr__( 'The content import step has been displayed', 'seowp' ) );
	}
	...
	<?php
		$this->logger->debug( esc_attr__( 'The final step has been displayed', 'seowp' ) );
		$this->logErrorMessage("-----------Theme content Completed----------");
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( esc_attr__( 'The existing child theme was activated', 'seowp' ) );
		...
		$this->logger->debug( esc_attr__( 'The newly generated child theme was activated', 'seowp' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'seowp' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'seowp' ),
				)
			);
		}
		...
		$this->logger->debug( esc_attr__( 'The license activation was performed with the following results', 'seowp' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'seowp' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'seowp' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'seowp' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'seowp' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'seowp' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'seowp' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'seowp' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( esc_attr__( 'The child theme functions.php content was generated', 'seowp' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( esc_attr__( 'The child theme style.css content was generated', 'seowp' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( esc_attr__( 'The child theme screenshot was copied to the child theme, with the following result', 'seowp' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( esc_attr__( 'The child theme screenshot was not generated, because of these results', 'seowp' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'seowp' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'seowp' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'seowp' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				esc_attr__( 'A plugin with the following data will be processed', 'seowp' ),
				array(
			...
			$this->logErrorMessage("Plugin Installed : ".$_POST['slug']. " ".$json['message']);

			

			$json['hash']    = md5( serialize( $json ) );
			$json['message'] = esc_html__( 'Installing', 'seowp' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				esc_attr__( 'A plugin with the following data was processed', 'seowp' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			$this->logErrorMessage("Succefully Installed : ".$_POST['slug']);

			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'seowp' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || ! current_user_can( 'install_themes' ) ) {
			$this->logger->error( esc_attr__( 'User has no right to perform an AJAX action', 'seowp' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'You have no right to perform this action', 'seowp' ),
				)
			);
		}
		...
		if(isset($_POST['content']) && $_POST['content'] == "template_import" && !isset($_POST['proceed'])){				
			update_option('is_element_templates_imported','1');
				update_option('is_element_templates_imported2','1');
				update_option('is_element_templates_imported3','1');
			update_option('is_element_templates_imported4','1');
		...
		if ( empty( $_POST['content'] ) || ! isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( esc_attr__( 'The content importer AJAX call failed to start, because of incorrect data', 'seowp' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'seowp' ),
				)
			);
		}
		...
		if($_POST['content'] && $_POST['proceed'] && $_POST['action'] == "merlin_content"){
		$this->logErrorMessage("Import start : ".$_POST['content']);
		}


		if($_POST['selected_index']){
			$type = "Flat";
		}else{
			$type = "Fresh";
		}
		$this->logErrorMessage("Selected Type ".$type);
			...
			if ( is_callable( $this_content['install_callback'] ) ) {
				if ( ! empty( $this_content['data'] ) ) {
					...
					if ( isset( $this_content['data'][ $next_to_import ]['file_path'] ) ) {
						...
						if	( ! isset( $this_content['data'][ $next_to_import ] ) ) {
				...
				$this->logger->info(
					esc_attr__( 'The content import AJAX call will be executed with this import data', 'seowp' ),
					array(
						'title' => $title,
						'data'  => $data,
					)
				...
				/**
				 * ERROR WARNING:
				 * When there is too many files to import, the function bellow
				 * won't finish but generate new ajax call inside and die().
				 * That makes the script to start over from the wrong position.
				 * This is why we need to save the current state before processing
				 * and come back to it in case we detec interuption.
				 */
				// Save current state:
				// update_option( '' );
		...
		if ( false === $json || -1 !== $next_to_import ) {
			...
			$json = array(
				...
				'num_of_imported_posts' => $num_of_imported_posts,
			);
		}
		...
		if ( $json ) {
			$json['hash'] = md5( serialize( $json ) );
			wp_send_json( $json );
			$this->logErrorMessage("Import :".$_POST['content']. " Success");

			// ☝️Send a JSON response back to an AJAX request, and die().
		} else {
			$this->logErrorMessage("Import :".$_POST['content']." Fail");

			$this->logger->error(
				esc_attr__( 'The content import AJAX call failed with this passed data', 'seowp' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'seowp' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		...
		if ( ! current_user_can( 'install_themes' ) || ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( esc_attr__( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'seowp' ) );
			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'seowp' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'seowp' ),
				'description'      => esc_html__( 'Demo content data.', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Installing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
		...
		if ( ! empty( $import_files['content-set'] ) ) {
			$content['content-set'] = array(
				'title'            => ( ! empty( $import_files['content'] ) ) ? esc_html__( 'Extra Content', 'seowp' ): esc_html__( 'Content', 'seowp' ),
				'description'      => esc_html__( 'Demo content data.', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Installing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'seowp' ),
				'description'      => esc_html__( 'Sample widgets data.', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Installing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'seowp' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Installing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'seowp' ),
				'description'      => esc_html__( 'Sample theme options data.', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Installing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
			...
			$content['template_import'] = array(
				'title'            => esc_html__( 'Elementor Templates', 'seowp' ),
				'description'      => esc_html__( 'Import Predefined Templates', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Importing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
				'install_callback' => array( 'Merlin_Customizer_Importer', 'import' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'seowp' ),
				'description'      => esc_html__( 'Redux framework options.', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Installing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'seowp' ),
				'description'      => esc_html__( 'After import setup.', 'seowp' ),
				'pending'          => esc_html__( 'Pending', 'seowp' ),
				'installing'       => esc_html__( 'Installing', 'seowp' ),
				'success'          => esc_html__( 'Success', 'seowp' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( esc_attr__( 'The revolution slider import was executed', 'seowp' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'seowp' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( esc_attr__( 'The home page was set', 'seowp' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( esc_attr__( 'The blog page was set', 'seowp' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( esc_attr__( 'The Hello world post status was set to draft', 'seowp' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( esc_attr__( 'This predefined demo import does not have the name parameter: import_file_name', 'seowp' ), $import_file );
			}
		}
	...
	public function update_selected_import_data_info() {
		...
		update_option('update_selected_import_data_info',$selected_index);
		...
		?>
			<?php foreach ( $import_info as $slug => $available ) : ?>
				...
				<li class="merlin__drawer--import-content__list-item status status--Pending" data-content="<?php echo esc_attr( $slug ); ?>">
					<input type="checkbox" name="default_content[<?php echo esc_attr( $slug ); ?>]" class="checkbox checkbox-<?php echo esc_attr( $slug ); ?>" id="default_content_<?php echo esc_attr( $slug ); ?>" value="1" <?= (esc_attr( $slug ) == "template_import")?'':'checked'; ?>>
