...
class Importer extends WXRImporter {
	...
	protected function get_reader( $file ) {
		...
		if ( ! class_exists( 'XMLReader' ) ) {
			$this->logger->critical( esc_attr__( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'seowp' ) );
		...
		if ( ! $status ) {
			$this->logger->error( esc_attr__( 'Could not open the XML file for parsing!', 'seowp' ) );
	...
	public function import( $file, $options = array() ) {
		...
		if ( is_wp_error( $result ) ) {
			$this->logger->error( esc_attr__( 'Content import start error:', 'seowp' ) . ' '  . $result->get_error_message() );
	...
	public function new_ajax_request_maybe( $data ) {
		...
		error_log( 'Importing: ' . $data['post_title'] );
		...
		if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 20 ) ) {
			...
			$this->logger->info( esc_attr__( 'New AJAX call!', 'seowp' ) );
		...
		if ( strstr( $data['taxonomy'], 'pa_' ) ) {
			if ( ! taxonomy_exists( $data['taxonomy'] ) ) {
				...
				*/
				...
				/* ✅ register_taxonomy() - Used in the demo content import library proteusthemes/one-click-demo-import to import the WooCommerce attributes properly. */
				/*
				register_taxonomy(
					$data['taxonomy'],
