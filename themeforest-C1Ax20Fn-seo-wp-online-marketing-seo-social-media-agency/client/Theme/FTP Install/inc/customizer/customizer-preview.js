...
( function( $ ) {
	...
	function updateElementFont (controlName, htmlAddress) {
		wp.customize( controlName, function( value ) {
			value.bind( function( to ) {
				// Here we create/replace style blocks with CSS for each controlName
				// One block per controlName
				// Creating style blocks instead of using inline css provide us
				// with possiblity to change :hover states live

				// Find if we already have data-stylesheet-id="controlName"
				// to make sure we do not create multiply blocks for each controlName
				...
				// Create style block if it was't created for controlName before
				if ( !style ) {
	...
	*/
	...
	// check state on load and show/hide .header based on state
	jQuery(document).ready(function ($) {
		if ( $('.site-header').data('stateonload') == ''  ) {
			$( '.site-header' ).css( 'display', 'none' );
		}
	});

	updateDisplayProperty ('lbmn_headertop_switch', '.site-header');
